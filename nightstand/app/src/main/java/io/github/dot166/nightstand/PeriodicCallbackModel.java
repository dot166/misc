...
import static io.github.dot166.nightstand.Utils.enforceMainLooper;
...
final class PeriodicCallbackModel {
    ...
    private static final class PeriodicRunnable implements Runnable {
        ...
        @Override
        public void run() {
            Log.i("NightStand", "Executing periodic callback for %s because the period ended" + mPeriod);
            mDelegate.run();
            schedule();
        }

        private void runAndReschedule() {
            Log.i("NightStand", "Executing periodic callback for %s because the time changed" + mPeriod);
            unSchedule();
            mDelegate.run();
            schedule();
        }
