...
import io.github.dot166.nexus.NexusOverlay;
...
public class LocalActivityManagerCompat {
     ...
     */
    public Window startActivity(String id, Intent intent, @NotNull NexusOverlay overlay) {
        ...
        if (adding) {
            ...
            if (aInfo == r.activityInfo ||
                ...
                if (aInfo.launchMode != ActivityInfo.LAUNCH_MULTIPLE ||
                    ...
                    if (r.activity instanceof ComponentActivity activity) {
    ...
    public void dispatchDestroy(boolean finishing) {
        ...
        for (int i=0; i<N; i++) {
            ...
            if(r.activity != null) {
                try {
                    Field field = Activity.class.getDeclaredField("mChangingConfigurations");
                    field.setAccessible(true);
                    field.set(r.activity, !finishing);
                } catch (Exception e) {
                    // ignore, lol
                }
            }
